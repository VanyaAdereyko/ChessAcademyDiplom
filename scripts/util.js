...
window.log = function() {
    log.history = log.history || []; //     хранить журналы в массиве для справки
...
function drawBoard(board){
    ...
    var showsDummyBoard = false; // при включении этого установите ширину доски на 800 пикселей в .css
...
function boardToFEN(board){
    ...
    for(var i=0; i< 128; i++){
        if(! (i & 0x88) ) {
            var n = board[i];
            if((n & 0x07) === 0x07){ // королева
                piece = 'Q';
            }else if((n & 0x06) === 0x06){ // ладья
                piece = 'R';
            }else if((n & 0x05) === 0x05){ // слон
                piece = 'B';
            }else if((n & 0x03) === 0x03){ // король
                piece = 'K';
            }else if((n & 0x02) === 0x02){ // рыцарь
                piece = 'N';
            }else if((n & 0x01) === 0x01){ // пешка
                piece = 'P';
            }
            
            if(n === 0){ // пустой квадрат
                piece = '';
                emptySquares++;
            }else{
